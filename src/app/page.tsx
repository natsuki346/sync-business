...
export default function HomePage() {
  ...
  const remaining = MAX_CHARS - newPostContent.length
  ...
  return (
    <div>
      ...
      <button
        onClick={openModal}
        className="fixed z-50 shadow-lg flex items-center justify-center rounded-full"
        style={{ right: 16, bottom: 112, width: 56, height: 56, backgroundColor: '#1D9E75' }}
      >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] bg-white flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            ...
            <span className="text-sm font-semibold text-gray-900">投稿</span>
            <button
              onClick={submitPost}
              disabled={!newPostContent.trim() || remaining < 0}
              className="text-sm font-semibold text-white px-4 py-1.5 rounded-full disabled:opacity-40 transition-opacity"
              style={{ backgroundColor: '#1D9E75' }}
            >
              投稿する
            </button>
          ...
          {/* Post area */}
          <div className="flex gap-3 px-4 pt-4 flex-1 overflow-y-auto">
            <div
              className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white text-sm flex-shrink-0"
              style={{ backgroundColor: currentUser.avatarColor }}
            >
              {currentUser.name.replace(/\s/g, '').slice(0, 2)}
            </div>
            <div className="flex-1 min-w-0">
              {activeTab === 'community' && (
                <select
                  ...
                  className="bg-gray-100 rounded-xl px-3 py-1.5 text-xs outline-none mb-3"
                >
                  {followingTagObjects.map(tag => (
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200">
            ...
            <span
              ...
              style={{
                color: remaining < 0 ? '#EF4444' : remaining <= 20 ? '#EAB308' : '#9CA3AF',
              }}
            >
              {remaining}
            </span>
