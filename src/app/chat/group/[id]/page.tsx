...
import { groupChats, people, currentUser } from '@/data/mock'
...
const initialMessages: Record<string, Message[]> = {
  'gc-1': [
    { id: 'm1', authorId: '1', content: '今週金曜のライブ、誰か行きますか？', time: '14:30' },
    { id: 'm2', authorId: '4', content: '行きたい！どこのライブ？', time: '14:31' },
    { id: 'm3', authorId: 'me', content: '渋谷のClub Quattroです！', time: '14:32' },
  ],
  'gc-2': [
    { id: 'm1', authorId: '1', content: '発酵食品のレシピ共有します！', time: '昨日' },
  ],
  'gc-3': [
    { id: 'm1', authorId: '1', content: 'キャリア相談乗りますよ〜', time: '月曜' },
  ],
}
...
export default function GroupChatPage({ params }: { params: { id: string } }) {
  const chat = groupChats.find(g => g.id === params.id)
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-gray-400">{chat?.members.length ?? 0}人のメンバー</div>
        </div>
      </div>
          ...
          return (
            <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
                {!isMe && (
                  <span className="text-[11px] text-gray-400 px-1">{author.name}</span>
                )}
                <div
